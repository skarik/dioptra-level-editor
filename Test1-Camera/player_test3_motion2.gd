...
var mSprintPressed : bool = false;
var mSprintReleased : bool = false;
var mSprintPressTime : float = 0.0;

var mDashing = false;
var mDashingTime = 0.0;
...
func _unhandled_input(event: InputEvent):
	if (event is InputEventMouseMotion):
		...
		if (event.is_pressed()):
			...
			mSprintPressed = false;
			mSprintReleased = true;
	return
...
func _physics_process(delta: float):
	...
	if (not mDashing):
		...
		if (possibleNextMotionFrict.length_squared() < possibleNextMotionAccel.length_squared() \
			or possibleNextMotionFrict.dot(possibleNextMotionAccel) < 0.0):
